...
var ElasticSearchSessionStore = function(options) {
  ...
  var defaults = {
    index: "express-session",
    typeName: "session",
    prefix: "",
    ttl: 60 * 60 * 24 * 14 * 1000, // 14 days
    pingInterval: 1000,
  };
...
async function clearOldIndices(client, index) {
  ...
  indices.forEach((existingIndex) => {
    let matches =
      existingIndex.match(/([0-9]{4})-([0-9]{2})-([0-9]{2})$/) || [];
    if (matches !== 4) {
      return;
    }
    let year = Number(matches[1]);
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (
        ...
        typeof res.body.hits == "undefined"
      ) {
        resolve(null);
      }
      if (
...
_.extend(ElasticSearchSessionStore.prototype, {
  ...
  set: async function(sid, sess, callback) {
    ...
    try {
      ...
      console.trace("sessionstore, error creating index", err);
      if (err && err.message.toLowerCase().indexOf("version") >= 0) {
        return callback(
          new Error("ConcurrencyError: Session was updated by someone else!")
        );
      }
      callback(err);
    }
